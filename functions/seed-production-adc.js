/**
 * Seed PRODUCTION Firestore — No service account key required.
 * Uses Firebase Application Default Credentials (set by `firebase login`).
 * Run: node seed-production-adc.js
 */

// DO NOT set FIRESTORE_EMULATOR_HOST — writes to production
...
const products = [
  ...
  // Test machine — 2 products (₹1 for easy testing)
...
async function seedProduction() {
  console.log('🌱 Seeding PRODUCTION Firestore (using Application Default Credentials)...\n');
  console.log('⚠️  Writing to REAL Firestore (not emulator)\n');
  ...
  try {
    ...
    console.error('\n❌ Error seeding production:', error.message);
    console.error('\nFull error:', error);
    process.exit(1);
  }
}
